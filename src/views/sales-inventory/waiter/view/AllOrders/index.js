...
// ** Store & Actions
import { getUserAllUtilitiesTransactions, getFilteredWaiterOrders } from '../../store/action'
...
// custom Header
const CustomHeader = ({ handleFilter, searchTerm, handlePerPage, rowsPerPage, downloadCSV, storeData, downloadPDF, waiterDetails }) => {
	return (
		<div className="invoice-list-table-header w-100 py-2">
			<Row>
				<Col lg="6" sm="12" className="d-flex align-items-center px-0 px-lg-1 mb-2">
					...
					<h3 className="d-none d-lg-block">{waiterDetails.fullName.split(' ')[0]}'s Orders</h3>
				</Col>
				<Col
					lg="3"
					sm="6"
...
const TransactionList = () => {
	...
	useEffect(() => {
		...
		dispatch(
			getFilteredWaiterOrders(store.waiterDetails.orders, {
				page: currentPage,
				perPage: rowsPerPage,
				q: searchTerm,
			})
	...
	const filteredData = store?.waiterDetails.orders?.filter((item) => item?.orderNumber?.toLowerCase() || item?.status?.toLowerCase())

	const CustomPagination = () => {
		const count = Math.ceil(store?.waiterDetails.orders.length / rowsPerPage)
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(store.waiterDetails.orders[0])
	...
	const downloadPDF = () => {
		...
		store.waiterDetails.orders.map((arr) => {
			doc.autoTable({
				styles: { halign: 'left' },
				theme: 'grid',
				columnStyles: {
		...
		doc.save(`${store.waiterDetails.firstName} ${store.waiterDetails.lastName}_orders.pdf`)
	}
	...
	const dataToRender = () => {
		...
		if (store.waiterDetails.orders.length > 0) {
			return store.waiterDetails.orders
		} else if (store.waiterDetails.orders.length === 0 && isFiltered) {
			return []
		} else {
			return store.waiterDetails.orders.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={store.waiterDetails.orders}
								downloadPDF={downloadPDF}
								searchTerm={searchTerm}
								waiterDetails={store.waiterDetails}
							/>
						}
					/>
				</div>
