...
import { getAllData, deleteWaiter } from '../store/action'
...
const handleDelete = async (id) => {
  ...
  return MySwal.fire({
    ...
    if (result.value) {
      const deleted = await store.dispatch(deleteWaiter(id))
      if (deleted.status) {
        await store.dispatch(getAllData())
          MySwal.fire({
              icon: 'success',
              title: 'Deleted!',
              text: 'Waiter record has been deleted.',
              customClass: {
                confirmButton: 'btn btn-primary'
              }
          })
