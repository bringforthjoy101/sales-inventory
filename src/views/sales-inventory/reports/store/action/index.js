...
export const getSalesReport = ({ startDate, endDate }) => {
	return async (dispatch) => {
		...
		const response = await apiRequest({ url: '/orders-report', method: 'POST', body }, dispatch)
		const res = await apiRequest({ url: '/orders-summary', method: 'POST', body }, dispatch)
		if (response) {
			if (response.data.data && response.data.status) {

				await dispatch({
					type: 'GET_ALL_REPORT_DATA',
					data: { orderslist: response.data.data, reportSummary: res.data.data }
				})
			} else {
